...
```{r stab1e4, fig.show = 'hide', include = FALSE, echo = FALSE}
set.seed(4321)
...
k0 <- as.data.table(scale(data01[sk0, 1:24, with = FALSE], center = TRUE, scale = FALSE))
w0 <- as.data.table(scale(datEx[sw0,], center = TRUE, scale = FALSE))

hmcLK <- p.hmc(k0, maxDim = 8, modelNames = c("VVV"))
hmcLW <- p.hmc(w0, maxDim = 8, modelNames = c("VVV"))
```

Here is the same test, but with 10,000 sample points for each plot.
```{r pstack1e4, echo = FALSE, fig.width = 12, fig.height = 5}
p5 <- p.stackM(hmcLK, ccol = ccol3, centered = TRUE)
p6 <- p.stackM(hmcLW, ccol = ccol3, centered = TRUE)
...
hk <- p.hmc(k1, modelNames = c("VVV"), maxDepth = 5, maxDim = 8)
hw <- p.hmc(w1, modelNames = c("VVV"), maxDepth = 5, maxDim = 8)

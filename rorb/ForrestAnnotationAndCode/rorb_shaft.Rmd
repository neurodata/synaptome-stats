...
def getSizeAnno(uniq):
    ...
    bb['id'] = uniq
    sizeAnno = np.sum(rem.get_cutout(anno_res, 0, bb['x_range'], bb['y_range'], bb['z_range'], id_list = [bb['id']]) == bb['id'])
    out = [bb['id'], sizeAnno]
    return(out)
...
dat$shaft <- shaft
dat$size <- ids$size
dat$um3 <- dat$size * 3*3*50 / 1000^3

df1 <- dat[, .(shaft, size, um3, PSD95)]

p <- ggplot(df1, aes(x = um3, y = PSD95, color = um3, shape = as.factor(shaft))) + geom_point() + xlab(expression(paste(mu, "m^3"))) # 

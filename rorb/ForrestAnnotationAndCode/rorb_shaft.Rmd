...
sdat$shaft <- shaft
sdat$size <- ids$size
sdat$um3 <- sdat$size * 3*3*50 / 1000^3

df1 <- sdat[, .(shaft =as.factor(shaft), size, um3, PSD95)]

p <- ggplot(df1, aes(x = um3, y = PSD95, color = shaft)) +
...
p2 + geom_density(aes(x = size, y = ..density.., col = as.factor(shaft))) + 
  ggtitle("size | shaft")

p2 + geom_density(aes(x = size, y = ..density.., col = as.factor(shaft))) + scale_x_log10() + 

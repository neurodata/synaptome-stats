...
mdim <- 1.5e3
mu1sig <- 1
trueBig <- c(rep(0,1e3), rep(1,1e3))

params <- list(means = c(1, rep(0,mdim-1)), Sigma = diag(1, mdim))

dlist <- c(1,seq(100, mdim, by = 100))

bigD <- 
foreach(i = 16:17 .combine = 'rbind') %:% 
foreach(x = 1:2, .combine = 'rbind') %do% {
  set.seed(i)
  w <- 
    rbind( 
          rmvnorm(1e3,  mean = c(1,rep(0,dlist[i]-1)), sigma = diag(2,dlist[i])),
          rmvnorm(1e3,  mean = -c(1,rep(0,dlist[i]-1)), sigma = diag(2,dlist[i]))
          )
  ...
  qdaD <- qda(trueBig ~ w, CV = TRUE)
        
  (Lr <- sum(rcl != trueBig)/length(trueBig))
  (Ld <- sum((as.numeric(qdaD$class) - 1) != trueBig)/length(trueBig))
  cbind(Lr, Ld, dim = i)
}

...
p1 <- plot(d1heat(snaL1)) 
p1
dev.off()

png("mlocation_L1.png", width = 720, height = 480)
p1 <- plot(mlocation(naL1, ccol = ccol)) 
p1
dev.off()
...
hL1 <- hmc(snaL1, modelNames = c("VVI"), ccol = ccol)
...
p1 <- stackM(hL1, ccol = ccol, depth = 3, centered = TRUE)
p1
dev.off()

png("clusterMeans_L1.png", width = w, height = 0.75*h)
p1 <- clusterMeans(hL1, ccol = ccol)
p1
dev.off()
```
...
p1 <- plot(d1heat(snaLinf)) 
p1
dev.off()

png("mlocation_Linf.png", width = 720, height = 480)
p1 <- plot(mlocation(naInf)) 
p1
dev.off()
...
hLinf <- hmc(snaLinf, modelNames = c("VVI"), ccol = ccol)
...
p1 <- stackM(hLinf, ccol = ccol, depth = 3, centered = TRUE)
p1
dev.off()

png("clusterMeans_Linf.png", width = w, height = 0.75*h)
p1 <- clusterMeans(hLinf, ccol = ccol)
p1
dev.off()
```

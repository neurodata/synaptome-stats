...
lsynF <- synF[!ans1, lapply(.SD, 
                            function(x){ 
                             as.integer(scale(log10(x),center=TRUE,scale=TRUE))
                            })]
...
```{r cc-F1-5,w=6*5,h=6*5,fig.cap=fig$cap("synF0-4","Scatter plots of Synapsin1 and Synapsin2 on linear and log scale.")}
do.call("grid.arrange",args=c(gg, ncol=3))
```

```{r pv1}
lm.fits1 <-lapply(gg,function(x){x <- x$data;lm(as.formula(paste0(names(x)[2],"~",names(x)[1])),data=x)})
r2 <- sapply(lm.fits1, function(x){ summary(x)$r.squared })
pval<- sapply(lm.fits1,function(x){ anova(x)$'Pr(>F)'[1] })

cbind(r2[1:6],pval[1:6], r2[7:12], pval[7:12], r2[13:18], pval[13:18])
```

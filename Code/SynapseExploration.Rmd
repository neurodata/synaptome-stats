...
aggp <- aggregate(fs,by=list(lab=L[[1]]),FUN=mean) # FIX THIS
...
plot3d(locs1$V1,locs1$V2,locs1$V3,
       col=rev(brewer.pal(8,"Blues"))[locs1$cluster],
       alpha=0.75,
       xlab='x', 
       ylab='y', 
       zlab='z')
...
```{r cc-agg2, eval=TRUE}
## Formatting data for heatmap
aggp2 <- aggregate(fs,by=list(lab=L[[2]]),FUN=function(x){mean(x)}) # FIX THIS
...
```{r synEx-kdeLV2, eval=TRUE,echo=TRUE,cache=FALSE,w=12,h=18,fig.cap=fig$cap("gKDE2","Kernel density estimates for each channel, on `fs` data given cluster from km++ level 2")}
print(gg3)
```
...
plot3d(locs2$V1,locs2$V2,locs2$V3,
       col=rev(brewer.pal(8,"Blues"))[locs2$cluster],
       alpha=0.75,
       xlab='x', 
       ylab='y', 
       zlab='z'
...
corLV3 <- lapply(c(1:6),function(x){cor(fs[L[[3]] == x,])})
...
```{r cc-agg3, eval=TRUE}
## Formatting data for heatmap
aggp3 <- aggregate(fs,by=list(lab=L[[3]]),FUN=function(x){mean(x)}) # FIX THIS
...
plot3d(locs3$V1,locs3$V2,locs3$V3,
       col=rev(brewer.pal(8,"Blues"))[locs3$cluster],
       alpha=0.75,
       xlab='x', 
       ylab='y', 
       zlab='z'
...
subid <- currentSubscene3d()
rglwidget(elementId="plot3dLocationsLV3")
```

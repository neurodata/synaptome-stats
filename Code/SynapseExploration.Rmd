...
```{r cc-msp}
exMat <- cmatfs[exType == 'ex',] -> g1
inMat <- cmatfs[exType == 'in',] -> g2
otMat <- cmatfs[exType == 'other',] -> g3
...
```{r cc-heatG123,eval=TRUE,w=6,h=6,fig.cap=fig$cap("heatg123", "Heatmaps of groups Ex, In, Ot" )}
...
```{r cc-BIC, eval=FALSE, echo=FALSE}
#bic on mdm1
    n <- nrow(mdm1)
    d <- ncol(mdm1)
    G <- 2

...
converting to z-scores or quantiles is required.  Also the data will be
filtered to the inner 98%.
...
### Taking log_10 on filtered data.
flog <- log10(fil98F0)
flogs <- scale(flog, center=TRUE, scale=TRUE)
...
- `fs`: Raw data scaled by subtracting mean and dividing by sample standard deviation, by column. 
- `flog`: $log_{10}$ of the scaled data.

### Kernel Density Estimates of the marginals

```{r cc_kde1, eval=TRUE, cache=FALSE,w=8,h=5,fig.cap=fig$cap("f0kde1","Kernel density estimates for each channel, on log_10 data.")}
...
gg1 <- ggplot(df, aes(x=value)) + 
    ...
    ggtitle("Kernel Density Estimates of log_10 data ")

print(gg1)
```

```{r cc_kde2, eval=FALSE, cache=FALSE,w=8,h=5,fig.cap=fig$cap("f0kde2","Kernel density estimates for each channel, on scaled º log_10 data.")}
df <- melt(as.matrix(flogs)) ## remove
names(df) <- c("ind","channel","value")
ts <- 22
gg2 <- ggplot(df, aes(x=value)) + 
    ...
    theme(plot.title=element_text(size=ts),
          ...
          strip.text=element_text(size=ts), 
          legend.position='none')+
    ggtitle("Kernel Density Estimates of scaled º log_10 transformed data ")
...
### Heat maps: log_10 data.

For the following we manualy choose 2 clusters.

```{r cc_agg, eval=TRUE}
## Formatting data for heatmap
feat2 <- aggregate(flog,by=list(lab=kvecflog[[2]]$cluster),FUN=mean)
feat2 <- as.matrix(feat2[,-1])

rownames(feat2) <- clusterFraction(kvecflog[[2]])

...
out <- foreach(i = 1:elbow,.combine='rbind') %:% 
       foreach(j = 1:25e4,.combine='rbind') %dopar% {
           set.seed(j)
           kv <- as.numeric(kmeans(pcaObj$x[,1:i],centers=K1)$cluster)
           
           data.frame(Embedding_Dimension=i,ARI=mclust::adjustedRandIndex(truth,kv))
           }
return(out)
}
```
...
```{r cc_ariPlots, w=4,h=4, fig.cap=fig$cap("ariPlot","ARI plot")}
...
```{r cc-ariVdim}
avdLog <- avd(pcaLog, elLog[2], truth)
avdLog$Embedding_Dimension <- as.factor(avdLog$Embedding_Dimension)
gg3 <- ggplot(data=avdLog, aes(x=ARI, y=..density..,color=Embedding_Dimension)) + 
        geom_histogram(binwidth=3.49*sd(avdLog$ARI)*length(avdLog$ARI)^(-1/3)) +
        facet_wrap(~Embedding_Dimension, scale='free')
print(gg3)
```

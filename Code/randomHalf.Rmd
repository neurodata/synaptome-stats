...
K <- 7
pamRaw5 <- pamk(pcaRaw$x[,1:elRaw[2]],krange=K,usepam=FALSE,critout=FALSE)
```
...
pamLog5 <- pamk(pcaLog$x[,1:elLog[2]],krange=K,usepam=FALSE,critout=FALSE)
...
pamRank5 <- pamk(pcaRank$x[,1:elRank[2]],krange=K,usepam=FALSE,critout=FALSE)
...
```{r ariRaw}
xRaw <- as.numeric(pamRaw5$pamo$clustering)
...
```{r ariLog}
xLog <- as.numeric(pamLog5$pamo$clustering)
...
```{r ariRank}
xRank <- as.numeric(pamRank5$pamo$clustering)

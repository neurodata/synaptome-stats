...
```{r pcaRaw, eval=TRUE, fig.cap=fig$cap("pcaRaw","Scree plot: Raw data")}
pcaRaw <- prcomp(fRaw,scale=FALSE, center=FALSE)
elRaw <- getElbows(pcaRaw$sdev) 
title("Scree plot: Raw data")
```
```{r pcaLog, eval=TRUE, fig.cap=fig$cap("pcaLog","Scree plot: Log data")}
pcaLog <- prcomp(fLog,scale=FALSE, center=FALSE)
elLog <- getElbows(pcaLog$sdev) 
title("Scree plot: Log data")
```
```{r pcaRank, eval=TRUE, fig.cap=fig$cap("pcaRank","Scree plot: Rank data")}
pcaRank <- prcomp(fRank,scale=FALSE, center=FALSE)
elRank <- getElbows(pcaRank$sdev)
title("Scree plot: Ranked data")
```

# PAMK

Running `pamk` for $K = 5$.

## PAMK on Raw data

```{r cc_pamkRaw, eval=TRUE}
pamRaw5 <- pamk(pcaRaw$x[,1:elRaw[2]],krange=5,usepam=FALSE,critout=FALSE)
```


```{r cc_aggRaw, eval=TRUE}
aggRaw <- aggregate(pcaRaw$x[,1:elRaw[2]],by=list(lab=pamRaw5$pamo$clustering),FUN=mean)

aggRaw <- as.matrix(aggRaw[,-1])
rownames(aggRaw) <- clusterFraction(pamRaw5$pamo$clustering)
```
...
```{r cc_pamkLog, eval=TRUE}
pamLog5 <- pamk(pcaLog$x[,1:elLog[2]],krange=5,usepam=FALSE,critout=FALSE)
```

```{r cc_aggLog, eval=TRUE}
aggLog <- aggregate(pcaLog$x[,1:elLog[2]],by=list(lab=pamLog5$pamo$clustering),FUN=mean)

aggLog <- as.matrix(aggLog[,-1])
rownames(aggLog) <- clusterFraction(pamLog5$pamo$clustering)
```
...
```{r cc_pamkRank, eval=TRUE}
pamRank5 <- pamk(pcaRank$x[,1:elRank[2]],krange=5,usepam=FALSE,critout=FALSE)
```

```{r cc_aggRank, eval=TRUE}
aggRank <- aggregate(pcaRank$x[,1:elRank[2]],by=list(lab=pamRank5$pamo$clustering),FUN=mean)

aggRank <- as.matrix(aggRank[,-1])
rownames(aggRank) <- clusterFraction(pamRank5$pamo$clustering)
```

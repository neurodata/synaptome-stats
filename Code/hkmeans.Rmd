...
```{r cc_kde1, eval=TRUE, cache=TRUE,w=8,h=4,fig.cap=fig$cap("f0kde1","Kernel density estimates for each channel, on untransformed data.")}
...
```{r cc_kde2, eval=TRUE, cache=TRUE,w=8,h=4,fig.cap=fig$cap("f0kde2","Kernel density estimates for each channel, on log transformed data.")}
...
```{r cc_kde3, eval=TRUE, cache=TRUE,w=8,h=4,fig.cap=fig$cap("f0kde3","Kernel density estimates for each channel, on log transformed scaled data.")}
...
```{r cc_kmeans,eval=FALSE,echo=TRUE}
...
```{r cc_level21, eval=FALSE, echo=FALSE}
fs11 <- fs[labs==1,]
fs12 <- fs[labs==2,]
...
rownames(feat5) <- c(sprintf("C1, %.2g",sum(labs==1)/length(labs)),sprintf("C2, %.2g", sum(labs==2)/length(labs)))
...
We begin by splitting the data into the two clusters as calculated
above.

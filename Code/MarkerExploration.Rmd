...
```{r cc-energyplot,h=17,w=12,fig.cap=fig$cap("enplt","F0-5: Distance correlation t-test statistic and p-value matrices.")}
...
for(i in 1:length(mdcor)){
  corrplot((log10(mdcor[[i]][[2]][ford,ford]+.Machine$double.eps)),is.corr=FALSE,method="color",tl.col=ccol[ford], 
           tl.cex=0.8, mar=c(1,0,1,1.5),
           p.mat=mdcor[[i]][[2]][ford,ford], sig.level=0.01)
  corrRect(tmp,col=Syncol,lwd=4)
  title(pTitle[i])
}
rm(tmp)
